...
<?php echo jumps($infos['authority'],CONTROLLER_NAME);?>
        ...
        <!--主要区域开始-->
        <div id="main">
            <form action="/index.php/Buy/index.html" method="get" id="search">
...
<?php if(is_array($category)): $i = 0; $__LIST__ = $category;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><option value="<?php echo ($vo["id"]); ?>" <?php if( I('get.category')==$vo['id'] ){ echo 'selected';} ?>  ><?php echo ($vo["name"]); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>                            
                    ...
                    <input type="button" value="增加" class="btn_add" onclick="location.href='/index.php/Buy/add';" />
...
<!--                        <td><?php echo ($vo["brand_name"]); ?></td>-->
                        ...
                        <td class="td_modi">                          
                            <input type="button" value="修改" class="btn_modify" onclick=" location.href='/index.php/Buy/edit/id/<?php echo ($vo["id"]); ?>'; " />
                            <input type="button" value="删除" class="btn_delete" onclick=" deletes( '/index.php/Buy/delete/id/<?php echo ($vo["id"]); ?>' ); " />                   
                        </td>
                        <td><a href="/index.php/Buy/detail/id/<?php echo ($vo["id"]); ?>">明细</a></td>
